    ...
    void Start()
    {
        ...
        if (resourcesPath != null)
        {
            ...
            // 바닥(Plane) 생성
                ...
                Vector3 planePosition = new Vector3(-9, -5, -11);
                GameObject plane = Instantiate(planePrefab, planePosition, Quaternion.identity);

                // Plane의 크기 설정 (X, Z 방향으로 기둥들의 범위에 맞춤)
                plane.transform.localScale = new Vector3(100, 1, 100); // Plane의 기본 크기는 10x10이므로, 10으로 나눔
        }
        else
        {
